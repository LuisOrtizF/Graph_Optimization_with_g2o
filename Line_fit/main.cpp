...
//      for example, a point on a line might be stored as Eigen::Vector2d,
...
class EdgePointOnLine : public g2o::BaseUnaryEdge<2, Eigen::Vector2d, VertexLine>
{
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        ...
        void computeError()
        {
            ...
            const VertexLine* v = static_cast<const VertexLine*>(_vertices[0]);

            //update _error using _measurement and v->estimate() 

            const double& m = v->estimate()(0);
            const double& b = v->estimate()(1);

            double yp = m * measurement().x() + b;
...
int main(int argc, char** argv)
{
    ...
    //-Add vertices
    VertexLine* vertices = new VertexLine();
    vertices->setId(0);
    // vertices->setEstimate(Eigen::Vector2d(0,0)); // es necesaria solo en un problema no lineal
    optimizer.addVertex(vertices);

    //-Add num_obs edges for the observations
    for(size_t i = 0; i < num_obs; i++)
    {
        EdgePointOnLine* e = new EdgePointOnLine;
    ...
    cout << "m      = " << vertices->estimate()(0) << endl;
